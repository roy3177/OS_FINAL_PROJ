...
namespace {
std::mutex g_mu; std::condition_variable g_cv; bool g_hasLeader = false; bool g_shutdown=false;
}
...
void lf_server_loop(int srv_fd, int workers)
{
    auto worker = [srv_fd]() {
        while (true) {
            ...
            g_cv.wait(lk, []{ return !g_hasLeader || g_shutdown; });
            ...
            lk.lock(); g_hasLeader = false; lk.unlock(); g_cv.notify_one();

            if (fd < 0) { if (errno == EINTR) continue; std::perror("accept"); return; }
...
static string trim_cr(string s){ if(!s.empty() && s.back()=='\r') s.pop_back(); return s; }
...
static string run_alg_or_error(const string& alg, const Graph& g, const unordered_map<string,int>& params, bool requestedDirected)
{
    ...
    if (!okForThisGraph) {
        std::ostringstream er; er << "Error: cannot run " << alg << " on " << (requestedDirected?"directed":"undirected") << " graph"; return er.str();
    }
...
static std::string serialize_graph_edges(const Graph& g, bool directed)
{
    ...
    if (directed) {
        for (int u=0; u<V; ++u) for (int v=0; v<V; ++v) if (cap[u][v] > 0) ++count;
    } else {
        for (int u=0; u<V; ++u) for (int v=u+1; v<V; ++v) if (cap[u][v] > 0 || cap[v][u] > 0) ++count;
    }
    out << "GRAPH " << V << " " << count << "\n";
    if (directed) {
        for (int u=0; u<V; ++u) for (int v=0; v<V; ++v) if (cap[u][v] > 0) out << "EDGE " << u << " " << v << " " << cap[u][v] << "\n";
    } else {
...
void handle_client(int fd)
{
    while (true)
    {
        std::string req; if (!recv_all_lines(fd, req)) { close(fd); return; }
        if (req == "EXIT\n" || req.find("\nEXIT\n") != string::npos) { send_response(fd, "BYE", true); close(fd); return; }

        // Parse request
    std::istringstream iss(req); string line; string alg; int V=-1; int E=0; int directed=0; int randomFlag=0; int seed=42; int src=-1,sink=-1,k=-1; int wmin=1,wmax=1;
        struct Edge{int u,v,w;}; vector<Edge> edges; bool parse_error=false; string perr;
        while (std::getline(iss,line)) {
            line = trim_cr(line);
            if (line=="END") break;
            if (line.rfind("ALG ",0)==0) { alg=line.substr(4); }
            else if (line.rfind("RANDOM ",0)==0) { std::istringstream ls(line); string t; ls>>t>>randomFlag; }
            else if (line.rfind("DIRECTED ",0)==0) { std::istringstream ls(line); string t; ls>>t>>directed; }
            else if (line.rfind("V ",0)==0) { std::istringstream ls(line); char t; ls>>t>>V; }
            else if (line.rfind("E ",0)==0) { std::istringstream ls(line); char t; ls>>t>>E; }
            else if (line.rfind("SEED ",0)==0) { std::istringstream ls(line); string t; ls>>t>>seed; }
            else if (line.rfind("EDGE ",0)==0) { std::istringstream ls(line); string t; int u,v,w=1; ls>>t>>u>>v; if (ls>>w){} edges.push_back({u,v,w}); }
            else if (line.rfind("WMIN ",0)==0) { std::istringstream ls(line); string t; ls>>t>>wmin; }
            else if (line.rfind("WMAX ",0)==0) { std::istringstream ls(line); string t; ls>>t>>wmax; }
            else if (line.rfind("PARAM ",0)==0) { std::istringstream ls(line); string t,kstr; int val; ls>>t>>kstr>>val; if(kstr=="SRC")src=val; else if(kstr=="SINK")sink=val; else if(kstr=="K")k=val; }
            else if (line.empty()) { continue; }
            else { parse_error=true; perr = string("Unknown directive: ")+line; break; }
        }
    if (parse_error) { send_response(fd, perr, false); continue; }
    if (V<=0) { send_response(fd, "Missing/invalid V", false); continue; }
    if (randomFlag && (E<0)) { send_response(fd, "Missing/invalid E", false); continue; }
        ...
        if (!randomFlag) {
            ...
            if (E>maxE) E=maxE; if (E<0) E=0;
            if (wmax < wmin) std::swap(wmax, wmin);
            g = generate_random_graph(V, E, seed, directed!=0, wmin, wmax);
        }

        unordered_map<string,int> params; if(src>=0)params["SRC"]=src; if(sink>=0)params["SINK"]=sink; if(k>=0)params["K"]=k;
...
{
    int port = PORT; if (argc>=2) { int p=std::atoi(argv[1]); if (p>0) port=p; }
    int srv = socket(AF_INET, SOCK_STREAM, 0); if (srv<0){ std::perror("socket"); return 1; }
    ...
    if (listen(srv, 64)<0){ std::perror("listen"); close(srv); return 1; }
