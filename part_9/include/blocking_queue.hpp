/*
Header-only thread-safe blocking queue for pipeline stages.
...
public:
	...
	bool push(const T& value) 
    {
		...
		if (closed_) return false; // reject pushes after close
		q_.push(value);
	...
	// Introspection helpers (non-blocking, approximate under concurrency)
	bool closed() const 
    {
		std::lock_guard<std::mutex> lk(mu_);
		return closed_;
